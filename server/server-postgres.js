...
app.post("/documents/:documentId/unarchive", authenticateToken, async (req, res) => {
  const documentId = Number(req.params.documentId);
  if (Number.isNaN(documentId)) {
    return res.status(400).json({ error: "Invalid document id" });
  }
  ...
  try {
    if (db.isPostgres()) {
      const unarchived = await db.unarchiveDocument(documentId);
      if (!unarchived) {
        return res.status(404).json({ error: "Document not found" });
      }
...
app.post("/documents/:documentId/extract", authenticateToken, async (req, res) => {
  const documentId = Number(req.params.documentId);
  if (Number.isNaN(documentId)) {
    return res.status(400).json({ error: "Invalid document id" });
  }
  ...
  try {
    ...
    if (db.isPostgres()) {
      ...
      zipBuffer = Buffer.isBuffer(doc.data) ? doc.data : Buffer.from(doc.data);
      parentEntity = doc.entityType;
      parentEntityId = Number(doc.entityId);
      parentFolderId = doc.parentId ?? null;
      ...
      zipBuffer = Buffer.from(doc.data || "", "base64");
      parentEntity = doc.entityType;
      parentEntityId = Number(doc.entityId);
      parentFolderId = doc.parentId ?? null;
      baseFilename = doc.filename;
    ...
    // Reject extracts whose total uncompressed size exceeds the per-file limit (zip-bomb guard).
    ...
    const baseFolderName = stripZipExtension(baseFilename);

    const ensureFolder = async (parentId, name) => {
      if (db.isPostgres()) {
        const created = await db.createDocumentFolder(parentEntity, parentEntityId, {
      ...
      store.documents.push(entry);
      if (!writeDb(store)) {
        throw new Error("Failed to persist extracted folder");
      }
      return Number(entry.id);
    };

    const writeFile = async (parentId, name, buffer) => {
      const filename = sanitizePathSegment(name);
      const mimeType = guessMimeTypeFromName(filename);
      if (db.isPostgres()) {
        await db.createDocument(parentEntity, parentEntityId, {
          filename,
          mimeType,
      ...
      store.documents.push(entry);
      if (!writeDb(store)) {
        throw new Error("Failed to persist extracted file");
      }
    };

    // Build the wrapper folder for the extraction
    const rootExtractFolderId = await ensureFolder(parentFolderId, baseFolderName);

    // Track folders we have created by their relative path inside the archive
    const folderIdByPath = new Map();
    folderIdByPath.set("", rootExtractFolderId);

    const ensurePathFolders = async (segments) => {
      ...
      for (const segment of segments) {
        ...
        const newId = await ensureFolder(currentParentId, segment);
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory) {
        await ensurePathFolders(segments);
        continue;
      }
      ...
      const targetParentId = folderSegments.length > 0
        ? await ensurePathFolders(folderSegments)
        : rootExtractFolderId;

      const fileBuffer = entry.getData();
      if (fileBuffer.length > MAX_DOCUMENT_SIZE_BYTES) {
        // Skip overly large extracted file but continue with the rest
        console.warn(`Skipping ${entry.entryName}: exceeds max size`);
        continue;
      }
      await writeFile(targetParentId, fileSegment, fileBuffer);
    }
...
app.get("/documents/public/:token", async (req, res) => {
  ...
  try {
    if (db.isPostgres()) {
      const doc = await db.getDocumentById(info.documentId, { includeData: true });
      if (!doc || !doc.data) {
  ...
  for (const type of entityTypes) {
    ...
    app.put(`${commissionsPath}/:id`, authenticateToken, async (req, res) => {
      try {
        ...
        if (!updated) return res.status(404).json({ error: "Not found" });
        await propagateLinkedFieldSync('commission', type, 'public', updated, payload);
        await propagateDealFieldSync('public', type, updated, payload);
        res.json(updated);
      } catch (error) {
  ...
  for (const type of entityTypes) {
    ...
    app.put(`${commissionsPath}/:id`, authenticateToken, async (req, res) => {
      try {
        ...
        if (apiPrefix === 'growth') {
          await propagateLinkedFieldSync('commission', type, 'growth', updated, payload);
        }
        await propagateDealFieldSync(apiPrefix, type, updated, payload);
...
app.post('/api/section-link/attach', authenticateToken, async (req, res) => {
  try {
    ...
    if (kind === 'commission') {
      ...
      const parentEntity = await db.getById(entityTable, Number(source.entity_id));
      if (!parentEntity) return res.status(400).json({ error: 'Parent entity not found' });

      let targetEntity = parentEntity.link_id
        ? await db.getByField(resolveTable('entity', type, targetNamespace), 'link_id', parentEntity.link_id)
        : null;

      if (!targetEntity) {
        const parentLinkId = parentEntity.link_id || crypto.randomUUID();
        const createdEntity = await createEntityCounterpart(type, targetNamespace, parentEntity, actorUserId);
        targetEntity = await db.setLinkId(resolveTable('entity', type, targetNamespace), createdEntity.id, parentLinkId);
        await db.setLinkId(entityTable, parentEntity.id, parentLinkId);
      }

      const commissionLinkId = source.link_id || crypto.randomUUID();
      const createdCommission = await createCommissionCounterpart(type, targetNamespace, targetEntity.id, source, actorUserId);
      const updatedCounterpart = await db.setLinkId(resolveTable('commission', type, targetNamespace), createdCommission.id, commissionLinkId);
      await db.setLinkId(sourceTable, source.id, commissionLinkId);

      return res.status(201).json({ linkId: commissionLinkId, counterpart: updatedCounterpart });
    }

    const linkId = source.link_id || crypto.randomUUID();
    const created = await createEntityCounterpart(type, targetNamespace, source, actorUserId);
    const updatedCounterpart = await db.setLinkId(resolveTable('entity', type, targetNamespace), created.id, linkId);
    await db.setLinkId(sourceTable, source.id, linkId);

    res.status(201).json({ linkId, counterpart: updatedCounterpart });
  } catch (error) {
  ...
  try {
    ...
    if (counterpart) {
      if (kind === 'entity') {
        ...
        await deleteCommissionCounterpart(type, targetNamespace, counterpart.id);
      }
    }
...
const findDealMembers = async (type, dealId) => {
  if (!dealId) return [];
  const members = [];
  for (const ns of DEAL_NAMESPACES) {
    const table = resolveTable('commission', type, ns);
    if (!table) continue;
    for (const row of await db.getAllByField(table, 'deal_id', dealId)) {
      members.push({ row, namespace: ns });
    }
  }
  return members;
};
...
const buildDealStatus = async (namespace, sourceType, sourceRow) => {
  const dealId = sourceRow.deal_id || null;
  const slots = {};
  for (const type of DEAL_TYPES) {
    let members = await findDealMembers(type, dealId);
    if (type === sourceType) {
      members = [
    ...
    for (const member of members) {
      slots[type].push(await buildDealSlot(type, member.namespace, member.row));
    }
  }
  return { dealId, slots };
};
...
const countDealMembers = async (dealId) => {
  ...
  let count = 0;
  for (const type of DEAL_TYPES) {
    count += (await findDealMembers(type, dealId)).length;
  }
  return count;
};

// Propagate core-field edits from a just-updated commission to every other
// participant of its deal, in whichever section each of them lives. A deal can
// hold several participants of the source's own type, so that type is walked
// too — skipping only the row the edit came from.
const propagateDealFieldSync = async (namespace, sourceType, updatedRow, incomingPayload) => {
  try {
    ...
    for (const targetType of DEAL_TYPES) {
      ...
      for (const member of await findDealMembers(targetType, updatedRow.deal_id)) {
...
app.get('/api/deal-link/status', authenticateToken, async (req, res) => {
  try {
    ...
    const source = await db.getById(resolveTable('commission', type, namespace), Number(id));
    if (!source) return res.status(404).json({ error: 'Not found' });
    res.json(await buildDealStatus(namespace, type, source));
  } catch (error) {
...
app.post('/api/deal-link/attach', authenticateToken, async (req, res) => {
  try {
    ...
    if (!source.deal_id) {
      await db.setDealId(sourceTable, source.id, dealId);
    ...
    const alreadyOnDeal = (await findDealMembers(targetType, dealId)).some(
      (member) => member.namespace === targetNamespace && Number(member.row.entity_id) === Number(targetEntity.id)
    );
    if (alreadyOnDeal) {
      return res.json(await buildDealStatus(namespace, type, await db.getById(sourceTable, source.id)));
    }
...
app.post('/api/deal-link/detach', authenticateToken, async (req, res) => {
  try {
    ...
    if (dealId) {
      ...
      const candidates = (await findDealMembers(targetType, dealId)).filter(
      ...
      for (const member of targeted) {
        await deleteCommissionCounterpart(targetType, member.namespace, member.row.id);
      }

      // If the deal has shrunk to a single commission, drop the grouping.
      if ((await countDealMembers(dealId)) < MIN_DEAL_MEMBERS) {
        for (const t of DEAL_TYPES) {
          for (const remaining of await findDealMembers(t, dealId)) {
            await db.setDealId(resolveTable('commission', t, remaining.namespace), remaining.row.id, null);
          }
        }
        source.deal_id = null;
      }
    ...
    const refreshedSource = await db.getById(sourceTable, source.id);
    res.json(await buildDealStatus(namespace, type, refreshedSource));
  } catch (error) {
...
const setSubjectIdOn = async (identity, subjectId) => {
  if ((identity.row.subject_id ?? null) === (subjectId ?? null)) return;
  await db.setSubjectId(resolveTable('entity', identity.type, identity.namespace), identity.row.id, subjectId);
 ...
 */
const collectSubjectIdentities = async (type, namespace, startRow) => {
  const found = new Map();
  ...
  while (queue.length > 0) {
    ...
    if (current.row.subject_id) {
      for (const role of SUBJECT_ROLES) {
        for (const ns of SUBJECT_NAMESPACES) {
          const table = resolveTable('entity', role, ns);
          if (!table) continue;
          for (const row of await db.getAllByField(table, 'subject_id', current.row.subject_id)) {
    ...
    if (current.row.link_id) {
      for (const ns of otherNamespaces(current.namespace)) {
        const table = resolveTable('entity', current.type, ns);
        if (!table) continue;
        const twin = await db.getByField(table, 'link_id', current.row.link_id);
 ...
 */
const collectSubjectLinkTwins = async (type, namespace, row) => {
  const twins = [{ type, namespace, row }];
  if (!row.link_id) return twins;
  for (const ns of otherNamespaces(namespace)) {
    const table = resolveTable('entity', type, ns);
    if (!table) continue;
    const twin = await db.getByField(table, 'link_id', row.link_id);
 ...
 * active view.
 */
const buildSubjectStatus = async (namespace, type, sourceRow, status) => {
  const identities = await collectSubjectIdentities(type, namespace, sourceRow);
  ...
  for (const role of SUBJECT_ROLES) {
    ...
    for (const identity of roleIdentities) {
      const table = resolveTable('commission', role, identity.namespace);
      if (!table) continue;
      for (const row of await db.getAllByField(table, 'entity_id', identity.row.id)) {
...
const createSubjectRoleRecord = async (type, namespace, sourceRow, targetType, targetNamespace, actorUserId) => {
  ...
  const createdTable = resolveTable('entity', targetType, targetNamespace);
  await db.setSubjectId(createdTable, created.id, subjectId);
  ...
  for (const identity of await collectSubjectIdentities(type, namespace, sourceRow)) {
    await setSubjectIdOn(identity, subjectId);
  }
  sourceRow.subject_id = subjectId;

  return await db.getById(createdTable, created.id);
};
 ...
 */
const resolveSubjectRoleRecord = async (type, namespace, sourceRow, targetType, targetNamespace, targetEntityId, actorUserId) => {
  if (targetEntityId !== undefined && targetEntityId !== null && targetEntityId !== '') {
    return await db.getById(resolveTable('entity', targetType, targetNamespace), Number(targetEntityId));
  }

  const candidates = (await collectSubjectIdentities(type, namespace, sourceRow))
  ...
  return await createSubjectRoleRecord(type, namespace, sourceRow, targetType, targetNamespace, actorUserId);
};

app.get('/api/subject-link/status', authenticateToken, async (req, res) => {
  try {
...
app.post('/api/subject-link/attach', authenticateToken, async (req, res) => {
  try {
    ...
    const source = await db.getById(resolveTable('entity', type, namespace), Number(id));
    if (!source) return res.status(404).json({ error: 'Not found' });
    const target = await db.getById(resolveTable('entity', targetType, targetNamespace), Number(targetEntityId));
    ...
    const group = [
      ...(await collectSubjectIdentities(type, namespace, source)),
      ...(await collectSubjectIdentities(targetType, targetNamespace, target)),
    ];
    const subjectId = source.subject_id || target.subject_id || crypto.randomUUID();
    for (const identity of group) await setSubjectIdOn(identity, subjectId);
    source.subject_id = subjectId;

    res.status(201).json(await buildSubjectStatus(namespace, type, source, status || null));
  } catch (error) {
...
app.post('/api/subject-link/create', authenticateToken, async (req, res) => {
  try {
    ...
    const created = await createSubjectRoleRecord(type, namespace, source, targetType, resolvedTargetNamespace, actorUserId);
    if (!created) return res.status(500).json({ error: 'Failed to create subject record' });

    const refreshed = await db.getById(resolveTable('entity', type, namespace), Number(id));
    res.status(201).json(await buildSubjectStatus(namespace, type, refreshed ?? source, status || null));
  } catch (error) {
...
app.post('/api/subject-link/detach', authenticateToken, async (req, res) => {
  try {
    const { namespace, type, id, targetType, targetNamespace, targetEntityId, status } = req.body || {};
    if (!isValidSubjectRequest({ namespace, type, id }) || !isSubjectRole(targetType) || !isLinkableNamespace(targetNamespace)) {
      return res.status(400).json({ error: 'Invalid subject-link request' });
    ...
    for (const twin of await collectSubjectLinkTwins(targetType, targetNamespace, target)) {
      await setSubjectIdOn(twin, null);
    }

    // One record left is not a group any more.
    const refreshedSource = await db.getById(resolveTable('entity', type, namespace), Number(id));
    const remaining = await collectSubjectIdentities(type, namespace, refreshedSource ?? source);
    if (remaining.filter((identity) => identity.row.subject_id).length < MIN_SUBJECT_IDENTITIES) {
      for (const identity of remaining) await setSubjectIdOn(identity, null);
    }

    const finalSource = await db.getById(resolveTable('entity', type, namespace), Number(id));
    res.json(await buildSubjectStatus(namespace, type, finalSource ?? source, status || null));
  } catch (error) {
 ...
 */
app.post('/api/subject-link/move-commission', authenticateToken, async (req, res) => {
  try {
    const {
      namespace, type, id,
    ...
    const subject = await db.getById(resolveTable('entity', type, namespace), Number(id));
    if (!subject) return res.status(404).json({ error: 'Not found' });
    const commission = await db.getById(resolveTable('commission', sourceType, sourceNamespace), Number(commissionId));
    if (!commission) return res.status(404).json({ error: 'Commission not found' });

    // Only the subject's own commissions may be moved through its panel.
    const identities = await collectSubjectIdentities(type, namespace, subject);
    ...
    await deleteCommissionCounterpart(sourceType, sourceNamespace, commission.id);

    const refreshedSubject = await db.getById(resolveTable('entity', type, namespace), Number(id));
    res.status(201).json({
      moved: {
        type: targetType,
        ...
        status: created.status ?? commission.status,
      },
      subject: await buildSubjectStatus(namespace, type, refreshedSubject ?? subject, status || null),
    });
  } catch (error) {
