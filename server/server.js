...
const getDocumentItemKind = (doc) => doc?.itemKind ?? (doc?.mimeType === DIRECTORY_MIME_TYPE ? "folder" : "file");
...
const stripDocumentData = (doc) => {
  ...
  return {
    ...
    parentId: normalizeDocumentParentId(doc.parentId),
...
app.post("/:entity/:id/document-folders", authenticateToken, (req, res) => {
  const entity = req.params.entity;
  const entityId = Number(req.params.id);
...
app.patch("/documents/:documentId", authenticateToken, (req, res) => {
  ...
  try {
    ...
    const hasParentUpdate = Object.prototype.hasOwnProperty.call(req.body ?? {}, 'parentId');
    if (hasParentUpdate) {
      const { parentId, error: parentError } = parseDocumentParentId(req.body.parentId);
      if (parentError) {
        return res.status(400).json({ error: parentError });
...
const createCommissionCounterpart = (db, type, targetNamespace, targetEntityInternalId, sourceRow, actorUserId, overrides = {}) => {
...
app.post('/api/deal-link/attach', authenticateToken, (req, res) => {
  try {
    ...
    const targetEntity = findSectionLinkRowById(getSectionLinkRows(db, 'entity', targetType, targetNamespace), targetEntityId);
    ...
    const createdResponse = createCommissionCounterpart(db, targetType, targetNamespace, targetEntity.id, source, actorUserId, {
      status: source.status || 'accepted',
    });
    const createdRow = findSectionLinkRowById(getSectionLinkRows(db, 'commission', targetType, targetNamespace), createdResponse.id);
