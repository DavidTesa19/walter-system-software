...
const expandCompanyRows = (rows: TiperGridRow[]): TiperGridRow[] =>
  rows.flatMap((row): TiperGridRow[] => {
  ...
  const groups: FieldGroup[] = [
    {
      ...
      fields: [
        { key: "name", label: "Jméno", value: entity.name, type: "text" },
        {
          ...
          label: "Organizace / Oblast působení",
...
const TipersSectionNew: React.FC<SectionProps> = ({
  ...
  const fetchData = useCallback(async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const entityOnlyRows: TiperGridRow[] = status !== "accepted"
        ? entitiesList
    ...
    setGridData((prev) => {
      ...
      for (const row of prev) {
        ...
        next.push(...expandCompanyRows([
          { ...row, ...mirror, entity: { ...row.entity, ...patch } as TiperEntity },
        ]));
      }
  ...
  const handleOpenRegionEditor = useCallback((data: TiperGridRow | undefined, anchor: FieldEditorAnchor) => {
    const entityId = data?.entity?.id ?? data?.tiper_entity_id ?? null;
    if (entityId == null) return;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getSectionLinkStatus("entity", "tiper", linkableNamespace, selectedEntity.id)
      .then((result) => {
        if (!cancelled) setEntityLinkedNamespaces(result.linkedNamespaces);
      })
      .catch((error) => {
        console.error("Error fetching tiper section-link status:", error);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getSectionLinkStatus("commission", "tiper", linkableNamespace, selectedCommission.id)
      .then((result) => {
        if (!cancelled) setCommissionLinkedNamespaces(result.linkedNamespaces);
      })
      .catch((error) => {
        console.error("Error fetching tiper commission section-link status:", error);
  ...
  const handleToggleEntitySectionLink = useCallback(async (targetNamespace: LinkableNamespace, checked: boolean) => {
    ...
    try {
      if (checked) {
        await attachSectionLink("entity", "tiper", linkableNamespace, selectedEntity.id, targetNamespace);
      } else {
        await detachSectionLink("entity", "tiper", linkableNamespace, selectedEntity.id, targetNamespace);
      }
      const result = await getSectionLinkStatus("entity", "tiper", linkableNamespace, selectedEntity.id);
  ...
  const handleToggleCommissionSectionLink = useCallback(async (targetNamespace: LinkableNamespace, checked: boolean) => {
    ...
    try {
      if (checked) {
        await attachSectionLink("commission", "tiper", linkableNamespace, selectedCommission.id, targetNamespace);
      } else {
        await detachSectionLink("commission", "tiper", linkableNamespace, selectedCommission.id, targetNamespace);
      }
      const result = await getSectionLinkStatus("commission", "tiper", linkableNamespace, selectedCommission.id);
  ...
  const updateProjectClusterStatus = useCallback(async (row: TiperGridRow, nextStatus: string) => {
    ...
    const linkedCommissions = commissions.filter((commission) => commission.tiper_entity_id === entityId);
  ...
  const updateProjectClusterWorkflowState = useCallback(async (row: TiperGridRow, nextState: string) => {
    const entityId = row.entity?.id ?? row.tiper_entity_id ?? null;
    if (entityId === null) {
      return;
    }
  ...
  const handleDelete = useCallback(async (id: number, options: { commissionOnly?: boolean } = {}) => {
    ...
    if (row?.entity && (row.subjectRow || row.entityOnly)) {
      ...
      if (isArchivedEntity) {
        ...
        confirmMessage = linkedCount > 0
          ? `Opravdu chcete zamítnout tohoto tipaře a všech ${linkedCount} navázaných tipů / zakázek?\n\nTipař: ${subjectLabel}\nID: ${row.entity_id}`
    ...
    try {
      ...
      if (linkTargetNamespaces.length > 0 && linkableNamespace && response?.entity?.id) {
        for (const targetNamespace of linkTargetNamespaces) {
          try {
            await attachSectionLink("entity", "tiper", linkableNamespace, response.entity.id, targetNamespace);
            if (response.commission?.id) {
              await attachSectionLink("commission", "tiper", linkableNamespace, response.commission.id, targetNamespace);
            }
          } catch (linkError) {
            console.error("Error linking tiper to other section:", linkError);
            alert(`Tipař byl vytvořen, ale propojení do sekce ${linkableNamespaceLabel(targetNamespace)} se nezdařilo.`);
          }
        }
      }
  ...
  const handleCreateEntityOnly = useCallback(async () => {
    ...
    try {
      ...
      if (linkTargetNamespaces.length > 0 && linkableNamespace && entity?.id) {
        for (const targetNamespace of linkTargetNamespaces) {
          try {
            await attachSectionLink("entity", "tiper", linkableNamespace, entity.id, targetNamespace);
          } catch (linkError) {
            console.error("Error linking tiper to other section:", linkError);
            alert(`Tipař byl vytvořen, ale propojení do sekce ${linkableNamespaceLabel(targetNamespace)} se nezdařilo.`);
          }
    ...
    <>
      ...
      {fieldEditor && fieldEditorEntity ? (
        <SubjectFieldPopover
          ...
          title={fieldEditor.kind === "region" ? "Kraj / Lokalita" : "Organizace / Oblast působení"}
          field={
